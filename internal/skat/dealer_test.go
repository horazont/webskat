...
func TestDeterministicDealerDeterministic(t *testing.T) {
	...
	t.Run("deal a full hand", func(t *testing.T) {
		...
		assert.Equal(t, Card{Card7, SuitBells}, drawn[0])
		assert.Equal(t, Card{Card8, SuitAcorns}, drawn[1])
		assert.Equal(t, Card{CardQueen, SuitAcorns}, drawn[2])

		deck, drawn, err = d.DrawCards(deck, 3)
		assert.Nil(t, err)
		assert.Equal(t, 23, len(deck))
		assert.Equal(t, Card{Card8, SuitHearts}, drawn[0])
		assert.Equal(t, Card{Card9, SuitLeaves}, drawn[1])
		assert.Equal(t, Card{Card8, SuitLeaves}, drawn[2])

		deck, drawn, err = d.DrawCards(deck, 2)
		assert.Nil(t, err)
		assert.Equal(t, 21, len(deck))
		assert.Equal(t, Card{CardKing, SuitBells}, drawn[0])
		...
		assert.Equal(t, Card{CardJack, SuitLeaves}, drawn[2])
		assert.Equal(t, Card{CardKing, SuitAcorns}, drawn[3])

		deck, drawn, err = d.DrawCards(deck, 4)
		assert.Nil(t, err)
		assert.Equal(t, 13, len(deck))
		assert.Equal(t, Card{CardAce, SuitLeaves}, drawn[0])
		assert.Equal(t, Card{Card10, SuitBells}, drawn[1])
		assert.Equal(t, Card{CardJack, SuitAcorns}, drawn[2])
		assert.Equal(t, Card{CardJack, SuitBells}, drawn[3])

		deck, drawn, err = d.DrawCards(deck, 4)
		assert.Nil(t, err)
		assert.Equal(t, 9, len(deck))
		assert.Equal(t, Card{Card9, SuitBells}, drawn[0])
		assert.Equal(t, Card{Card7, SuitAcorns}, drawn[1])
		assert.Equal(t, Card{CardJack, SuitHearts}, drawn[2])
		assert.Equal(t, Card{CardKing, SuitLeaves}, drawn[3])
	})
}
